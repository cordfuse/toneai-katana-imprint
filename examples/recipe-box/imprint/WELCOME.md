...
```bash
EXISTING_VERSION=$(defaults read ~/Desktop/Chef\ Remy.app/Contents/Info.plist ImprintVersion 2>/dev/null)
EXISTING_PATH=$(defaults read ~/Desktop/Chef\ Remy.app/Contents/Info.plist ImprintPath 2>/dev/null)
```

**Linux** — grep metadata from the .desktop file:
```bash
EXISTING_VERSION=$(grep '^X-Imprint-Version=' ~/Desktop/Chef\ Remy.desktop 2>/dev/null | cut -d= -f2)
EXISTING_PATH=$(grep '^X-Imprint-Path=' ~/Desktop/Chef\ Remy.desktop 2>/dev/null | cut -d= -f2)
```
