...
```bash
EXISTING_VERSION=$(defaults read ~/Desktop/Pairbot.app/Contents/Info.plist ImprintVersion 2>/dev/null)
EXISTING_PATH=$(defaults read ~/Desktop/Pairbot.app/Contents/Info.plist ImprintPath 2>/dev/null)
```

**Linux** — grep metadata from the .desktop file:
```bash
EXISTING_VERSION=$(grep '^X-Imprint-Version=' ~/Desktop/Pairbot.desktop 2>/dev/null | cut -d= -f2)
EXISTING_PATH=$(grep '^X-Imprint-Path=' ~/Desktop/Pairbot.desktop 2>/dev/null | cut -d= -f2)
```

**Windows** — read the Description field from the .lnk file (pipe-delimited `Imprint|version|path`):
```powershell
$WshShell = New-Object -ComObject WScript.Shell
$Existing = $WshShell.CreateShortcut("$env:USERPROFILE\Desktop\Pairbot.lnk")
...
**macOS** — create a native `.app` bundle at `~/Desktop/Pairbot.app`:

1. Create the directory structure:
```bash
mkdir -p ~/Desktop/Pairbot.app/Contents/MacOS
mkdir -p ~/Desktop/Pairbot.app/Contents/Resources
```

2. Create the launch script at `~/Desktop/Pairbot.app/Contents/MacOS/launch`:
```bash
#!/bin/bash
...
3. Create `~/Desktop/Pairbot.app/Contents/Info.plist` with version and path metadata:
```xml
...
iconutil -c icns /tmp/app.iconset -o ~/Desktop/Pairbot.app/Contents/Resources/icon.icns 2>/dev/null
rm -rf /tmp/app.iconset /tmp/app-icon.png
```

5. Refresh icon: `touch ~/Desktop/Pairbot.app`

If icon conversion fails, the app still works — just without a custom icon.

**Linux** — create `~/Desktop/Pairbot.desktop` with version and path metadata:
```ini
[Desktop Entry]
Type=Application
Name=Pairbot
...
> **Pairbot**: I put a **Pairbot** shortcut on your desktop — next time just double-click it. Hey! I'm Pairbot, your pair programming assistant. Which project are you working in today?
...
> **Pairbot**: I need access to the project directory to get started. Please make sure I'm running in the right location.
