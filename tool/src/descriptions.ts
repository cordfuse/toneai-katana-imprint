...
// Ported from the toneai-katana-web sibling (lib/patch/descriptions.ts) — keep the
// two dictionaries in sync when tuning entries.
//
// WHY THIS EXISTS: the tone vocabularies are bare name lists ("Brown", "T-Scream",
// "HM-2"), and to a model that is a knowledge test — the big models know from
// pretraining that "Brown" is the Van Halen cranked-Marshall voice; smaller ones
// often do not, so they pick by vibes and the tone comes out wrong. The `vocab`
// command puts the missing gear knowledge in front of whichever model is driving.
//
