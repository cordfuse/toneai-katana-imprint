...
export function buildToneSchema(vocab: DeviceVocab): Record<string, unknown> {
  const ampChannel = {
    ...
    properties: {
      type: { type: 'string', enum: [...vocab.amps], description: 'Amp model — run `vocab <device>` for what each voice is and when to reach for it.' },
  ...
  return {
    ...
    properties: {
      ...
      booster: {
        ...
        properties: {
          on: { type: 'boolean' },
          type: { type: 'string', enum: [...vocab.boosters], description: 'Overdrive/distortion/booster voicing — run `vocab <device>` for what each is and when to reach for it.' },
